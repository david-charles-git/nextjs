...
const BrandArchive = async () => {
	const postDetails: BrandArchiveDetails = await getBrandArchiveData();

	return (
		<main className="BrandArchive">
			<div className="page">
				<section className="intro grid">
					...
					<h1 className="clr-white">
						<TiltWrapper className="bclr-black" rotation={4} skew={4}>
							{postDetails.intro?.title || BrandArchiveModel.intro.title}
						</TiltWrapper>
					</h1>

					<p className="font-xl" dangerouslySetInnerHTML={{ __html: postDetails.intro?.copy || BrandArchiveModel.intro.copy }} />

					<LinkButton href={"/brand"} className="clr-white font-bold">
						View work
					</LinkButton>
				</section>
